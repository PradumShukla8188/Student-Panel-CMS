...
const Header: React.FC = () => {
  ...
  // Click outside for modals and menu
  const handleClickOutside = (event: MouseEvent) => {
  ...
  // Disable scroll when modals or menu open
  useEffect(() => {
    if (isSignInOpen || isSignUpOpen || navbarOpen) {
      document.body.style.overflow = 'hidden'
    } else {
  ...
  const handleMobileMenuClick = () => {
    setNavbarOpen(false) // automatically close sidebar
  }
    ...
    >
      <div className='container mx-auto max-w-7xl px-4 flex items-center justify-between'>
        ...
        >
          ...
          <nav className='flex flex-col items-start p-4'>
            {headerData.map((item, index) => (
              <div key={index} onClick={handleMobileMenuClick}>
            ...
            <div className='mt-4 flex flex-col gap-4 w-full'>
              <button
                className='bg-primary text-white px-4 py-2 rounded-lg border border-primary hover:text-primary hover:bg-transparent transition duration-300 ease-in-out'
                onClick={() => {
                  setIsSignInOpen(true)
                  setNavbarOpen(false)
                }}
